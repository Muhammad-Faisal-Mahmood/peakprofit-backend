...
const AffiliateApplications = require("../affiliate/affiliateApplication.model");

async function bcryptPassword(password) {
  return await bcrypt.hash(password, 10);
}

async function getAffiliateStatusByUserId(userId) {
  try {
    const application = await AffiliateApplications.findOne({ userId }).select(
      "status"
    );
